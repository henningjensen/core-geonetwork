...
GeoNetwork.Templates.TITLE = '<h1><input type="checkbox" <tpl if="selected==\'true\'">checked="true"</tpl> ' +
'class="selector" onclick="javascript:catalogue.metadataSelect((this.checked?\'add\':\'remove\'), [\'{uuid}\']);"/>' +
'<a href="#" onclick="javascript:catalogue.metadataShow(\'{uuid}\');return false;"' + 'title="{[values.abstract.substring(0, 350)]}">' +
'{title}</a>' + '<span class="md-action-menu"> - <a rel="mdMenu">' + OpenLayers.i18n('mdMenu') + '</a></span></h1>';
...
GeoNetwork.Templates.GEONORGE = new Ext.XTemplate(
		...
		'<thead><tr>',
		'<th></th><th>Tittel</th><th>Organisasjon</th><th>Ressurstype</th><th>Lenker</th><th>Produktinformasjon</th><th>Tegneregler</th>',
		'</tr></thead><tbody>',
		'<tpl for=".">',
		'<tr class="md md-full md-geonorge" style="{featurecolorCSS}">',
  		  '<td class="left w75" >', GeoNetwork.Templates.LOGO, '</td>',
	      '<td id="{uuid}">', GeoNetwork.Templates.TITLE, '</td>',
	      '<td class="w250">',
		      '<tpl if="contact !== undefined && contact.length != 0">',
			      '<tpl for="contact[0]">',
		          	'<h1>{name}</h1>',
		          '</tpl>',
		      '</tpl>',
	      ...
	      '<td class="w75">',
	          '<tpl if="type==\'dataset\'">',
	          	'<p>Datasett</p>',
	          '</tpl>',
	          '<tpl if="type==\'service\'  || type == \'service-view\'">',
	          	'<p>Tjeneste</p>',
	          '</tpl>',
	          '<tpl if="type==\'series\'">',
	          	'<p>Serie</p>',
	          '</tpl>',
	          '<tpl if="type!=\'dataset\' && type!=\'service\' && type!=\'series\' && type!=\'service-view\'">',
	          	'<p>{type}</p>',
	          '</tpl>',
	      '</td>',
          '<td class="md-links w150">',
        	'<ul>',
        	'<tpl for="links">',
                '<tpl if="parent.dynamic==\'true\' && (values.type == \'application/vnd.ogc.wms_xml\' || values.type == \'OGC:WMS\')">',
                    '<li><a href="#" class="md-mn addLayer" alt="Legg til WMS-lag på kartet" onclick="app.switchMode(\'1\', true);app.getIMap().addWMSLayer([[\'{[escape(values.title)]}\', \'{href}\', \'{name}\', \'{id}\']]);">Vis i kart&nbsp;</a></li>',
                '</tpl>',
                '<tpl if="parent.dynamic==\'true\' && values.type == \'application/vnd.google-earth.kml+xml\'">',
                    '<li><a href="{href}" class="md-mn md-mn-kml" alt="Open kml">KML</a></li>',
                '</tpl>',
                '<tpl if="parent.download==\'true\' && (values.type == \'application/zip\' || values.type == \'application/x-compressed\')">',
                    '<li><a href="{href}" class="md-mn md-mn-zip" alt="Download">Last ned data&nbsp;</a></li>',
                '</tpl>',
                '<tpl if="parent.download==\'true\' && values.protocol.indexOf(\'WWW:DOWNLOAD\')!=-1">',
                    '<li><a href="{href}" class="md-mn md-mn-zip" alt="Download">Last ned data&nbsp;</a></li>',
                '</tpl>',
                '<tpl if="values.type == \'text/html\'">',
                    '<li><a href="{href}" class="md-mn md-mn-www" alt="Web link" target="_blank">Gå til nettside&nbsp;</a></li>',
                '</tpl>',                    
            '</tpl>',
            '<tpl if="produktside">',
                '<li><a href="{produktside}" target="_blank" class="md-mn md-mn-www" alt="Gå til produktside">Produktside</a></li>',
            '</tpl>',
            '</ul>',
	      '</td>',
	      '<td class="md-links w250">',
	      	'<ul>',
	      	'<tpl if="produktspesifikasjon">',
	      		'<li><a href="{produktspesifikasjon}"  target="_blank" class="md-mn md-mn-zip" alt="Last ned produktspesifikasjon">Produktspesifikasjon</a></li>',
	      	'</tpl>',
	      	'<tpl if="produktark">',
	      		'<li><a href="{produktark}" target="_blank" class="md-mn md-mn-zip" alt="Last ned produktark">Produktark</a></li>',
	      	'</tpl></ul>',
	      	'</td>',
	      	'<td class="md-links w75"><ul>',
	      	'<tpl if="tegnforklaring">',
	      		'<li><a href="{tegnforklaring}" target="_blank" class="md-mn md-mn-zip" alt="Last ned tegnforklaring">Tegnforklaring</a></li>',
	      	'</tpl>',
	      	'</ul>',
	      '</td>',
		'</tr>',
